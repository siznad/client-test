$(function() {
  ...
  $.getJSON("./voice-token")
    .then(function(data) {
      ...
      device.on("connect", function(conn) {
        ...
        document.getElementById("button-complete").style.display = "inline";
  ...
  // Bind button to complete task
  document.getElementById("button-complete").onclick = function() {
    $.getJSON("./complete-task", { currentTask: currentTask })
    ...
    document.getElementById("button-call").style.display = "inline";
    document.getElementById("button-complete").style.display = "none";
  };
  ...
  log("Requesting Taskrouter Token...");
  // Using a relative link to access the Taskrouter Token function
  $.getJSON("./taskrouter-token")
    .then(function(tsdata) {
      log("Got a taskrouter token.");
      console.log("Token: " + tsdata.token);

      var worker = new Twilio.TaskRouter.Worker(tsdata.token);

      worker.on("ready", function(worker) {
        log("Twilio.Worker Ready!");
      ...
      worker.on("reservation.created", function(reservation) {
        ...
        currentTask = reservation.task.sid
        currentWS = reservation.task.workspaceSid
        console.log(reservation.sid)                  // WRxxx
        reservation.dequeue("+19726452282", "WA15825c257a45583259aeeae084fe2837");
      });
    })
    .catch(function(err) {
      console.log(err);
